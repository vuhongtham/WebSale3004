...
namespace WebSale.Data.Infrastructure
{
    ...
    {
        #region Properties
        private WebSaleDbContext dataContext;
        ...
        protected WebSaleDbContext DbContext
        {
        ...
        {
            //Handle includes for associated objectc if applicable
            if (includes != null && includes.Count() > 0)
            {
                var query = dataContext.Set<T>().Include(includes.First());
                foreach (var include in includes.Skip(1))
        ...
        public virtual IEnumerable<T> GetMultiPaging(Expression<Func<T, bool>> predicate, out int total, int index = 0, int size = 20, string[] includes = null)
        {
            ...
            //Handle includes for associated if applicable
            if(includes != null&& includes.Count() > 0)
            {
                var query = dataContext.Set<T>().Include(includes.First());
                ...
                _resetSet = predicate != null ? query.Where<T>(predicate).AsQueryable() : dataContext.Set<T>().AsQueryable();
            }
            else
            {
